...
class Wechat
{
	...
	const MESSAGE_SENDTEMPLATE_URL = '/message/template/send?';
	const QR_SCENE = 0;
	...
	*/
	...
	public function checkAuth($appid='',$appsecret=''){
		...
		if ($result)
		{
			...
			$this->cache('access_token', $json['access_token'], $expire);
	...
	*/
	...
	function uploadArticle($articles = array()) {
		...
		$result = $this->http_post(self::API_URL_PREFIX . self::UPLOAD_ARTICLE_URL . 'access_token=' . $this->access_token, self::json_encode($articles));
		if($result) {
			$json = json_decode($result, true);
			if(!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
			...
			//{"type": "news", "media_id": "dasdxxa", "created_at": "1391857799"}
			return $json;
		}
		return false;
	}
	/**
	 * 根据分组群发消息
	 * @param int $media_id 图文消息的media id
	 * @param int $group_id 分组ID
	 * @return false|array
	 */
	function massSendByGroup($media_id, $group_id) {
		if(empty($media_id) || empty($group_id)) return false;
		if (!$this->access_token && !$this->checkAuth()) return false;
		$data = array(
		...
		$result = $this->http_post(self::API_URL_PREFIX . self::MESSAGE_SENDGROUP_URL . 'access_token=' . $this->access_token, self::json_encode($data));
		if($result) {
			$json = json_decode($result, true);
	 ...
	 * 根据用户openid群发消息
	 * @param int $media_id 图文消息的media id
	 * @param array $openid_list 群发用户的openid列表
	 * @return false|array
	 */
	function massSendByOpenId($media_id, $openid_list) {
		if(empty($media_id) || !is_array($openid_list) || empty($openid_list)) return false;
		if (!$this->access_token && !$this->checkAuth()) return false;
		$data = array(
			'touser' => $openid_list,
			'mpnews' => array('media_id' => $media_id),
		...
		$result = $this->http_post(self::API_URL_PREFIX . self::MESSAGE_SENDUSER_URL . 'access_token=' . $this->access_token, self::json_encode($data));
		if($result) {
			$json = json_decode($result, true);
			if(!$json || $json['errcode'] != 0) {
				$this->errCode = $json['errcode'];
	...
	*/
	 ...
	 * @param string|int $state 重定向后所带参数,a-zA-Z0-9
